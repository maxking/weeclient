...
Weechats package allow decoding a message from weechat relay.
...
WeechatObjects are currently naive in implementation and uses
...
hpath, we created a wrapper around the value and Hda objects values
are of WeechatHdaValue type. For now, it only has a Value and
Hpath attributes, but in future we might add other fields if
needed or even remove it. All the parsing related code lives in
protocol.go
...
the Core event types in Weechat:
https://weechat.org/files/doc/stable/weechat_relay_protocol.en.html#message_identifier.
